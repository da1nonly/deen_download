...
class DLM_Admin_Writepanels {
			...
			<div class="dlm-metaboxes downloadable_files">
				<?php
					...
					if ( $files ) foreach ( $files as $file ) {
						...
						$file_type           = ( $file_type = get_post_meta( $file->ID, '_file_type', true ) ) ? $file_type : '';
						$file_name           = ( $file_name = get_post_meta( $file->ID, '_name', true ) ) ? $file_name : '';
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		if ( isset( $_POST['downloadable_file_id'] ) ) {
			...
			$downloadable_file_type         = $_POST['downloadable_file_type'];
			...
			for ( $i = 0; $i <= max( array_keys( $downloadable_file_id ) ); $i ++ ) {
				...
				$file_type           = strtolower( sanitize_text_field( $downloadable_file_type[ $i ]) );
				...
				// Update post meta
				update_post_meta( $file_id, '_name', $file_name );
				...
				if ( $main_file_path ) {
					...
					$filetype = $download_monitor->check_media_type( $main_file_path );
					$hashes   = $download_monitor->get_file_hashes( $main_file_path );
					update_post_meta( $file_id, '_file_type', $filetype );
