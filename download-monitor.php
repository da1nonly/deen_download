<?php
/*
Plugin Name: Deen ul islam - Download
Plugin URI: http://deen-ul-islam.org/
Description: A full solution for managing downloadable files, monitoring downloads and outputting download links and file information on your WordPress powered site.
Version: 1.5.1
Author: Deen ul Islam
Author URI: http://deen-ul-islam.org
...
class WP_DLM {
    ...
    public function init_taxonomy() {
        ...
        register_post_type( "dlm_download",
            apply_filters( 'dlm_cpt_dlm_download_args', array(
                    'labels' => array(
                        'all_items'          => __( 'All Media', 'download-monitor' ),
                        'name'               => __( 'Media', 'download-monitor' ),
                        'singular_name'      => __( 'Media', 'download-monitor' ),
                        'add_new'            => __( 'Add New', 'download-monitor' ),
                        'add_new_item'       => __( 'Add Media', 'download-monitor' ),
                        'edit'               => __( 'Edit', 'download-monitor' ),
                        'edit_item'          => __( 'Edit Media', 'download-monitor' ),
                        'new_item'           => __( 'New Media', 'download-monitor' ),
                        'view'               => __( 'View Media', 'download-monitor' ),
                        'view_item'          => __( 'View Media', 'download-monitor' ),
                        'search_items'       => __( 'Search Media', 'download-monitor' ),
                        'not_found'          => __( 'No Media found', 'download-monitor' ),
                        'not_found_in_trash' => __( 'No Media found in trash', 'download-monitor' ),
                        'parent'             => __( 'Parent Media', 'download-monitor' )
                    ),
        ...
        register_post_type( "dlm_download_version",
            apply_filters( 'dlm_cpt_dlm_download_version_args', array(
                    'labels' => array(
                        'all_items'          => __( 'All Files', 'download-monitor' ),
                        'name'               => __( 'Files', 'download-monitor' ),
                        'singular_name'      => __( 'File', 'download-monitor' ),
                        'add_new'            => __( 'Add New', 'download-monitor' ),
                        'add_new_item'       => __( 'Add File', 'download-monitor' ),
                        'edit'               => __( 'Edit', 'download-monitor' ),
                        'edit_item'          => __( 'Edit File', 'download-monitor' ),
                        'new_item'           => __( 'New File', 'download-monitor' ),
                        'view'               => __( 'View File', 'download-monitor' ),
                        'view_item'          => __( 'View File', 'download-monitor' ),
                        'search_items'       => __( 'Search Files', 'download-monitor' ),
                        'not_found'          => __( 'No Files found', 'download-monitor' ),
                        'not_found_in_trash' => __( 'No Files found in trash', 'download-monitor' ),
                        'parent'             => __( 'Parent Files', 'download-monitor' )
                    ),
                    'public'     => true,
                    'show_ui'     => true,
                    'menu_position'         => 6,
     ...
     * @return type
     */
    public function check_media_type($file_path) {
        if (strpos($file_path, 'youtube') > 0) {
            $file_type = 'youtube';
        } elseif (strpos($file_path, 'vimeo') > 0) {
            $file_type = 'vimeo';
        } else {
            $new_url   = current( explode( '?', basename( $file_path ) ) );
            $file_type = strtolower( substr( strrchr( $new_url, "." ), 1 ) );
        }
     ...
     * @return type
     */
    public function get_youtube($file_path) {

        # get video id from url
        $urlQ = parse_url( $file_path, PHP_URL_QUERY );
    ...
    /**
     * youtube duration function.
     *
     * @access public
     * @return type
     */
    public function get_youtube_duration($file_path) {
        $yt_time = get_youtube($file_path);
        $start   = new DateTime('1970-01-01');
        
        $start->add(new DateInterval($yt_time->duration));
        
        $youtube_duration = $start->format('H:i:s');
